...
Inheritance chain hint (base classes extracted from header):
!{grep -E ":\s*(public|protected|private)\s+\w+" CommonLibVR/include/RE/{{args}}.h | grep -oE "(public|protected|private)\s+\w+" | awk '{print $2}' | sort -u}

---

## Verification Protocol

1. **Rust file** — already injected above.

2. **C++ header** — already injected above.

3. **C++ implementation** — already injected above. MANDATORY even if it seems complete.

4. **Recursively trace the FULL C++ inheritance chain to root.**
   The base classes are listed in the inheritance chain hint above.
   For each class in the chain: read its `.h` AND `.cpp` before verifying.
   Repeat recursively until `TESForm`, `BSIntrusiveRefCounted`, or another
   known root is reached.
