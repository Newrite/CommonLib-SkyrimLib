//!
//! Defines the global memory allocator, and exposes it for other crates.
//!
//! Additionally, Implements thin wrappers around cstdio and the win32 library, allowing files
    ...
    impl File {
        ...
        pub fn wopen(path: &WideStr, mode: &WideStr) -> Result<File, ()> {
            unsafe {
                ...
                _wfopen(path.as_ptr(), mode.as_ptr())
                    .map(|f| Self(f))
                    .ok_or(())
            }
        }
        ...
        /// Consumes the contents of the file, reading all of it into a vector
        ...
        /// Consumes the contents of the file, reading all of it into a string
        /// and returning the result.
        pub fn into_string(self) -> Result<String, ()> {
            let bytes = self.into_vec()?;
            if bytes.len() >= UTF16LE_BOM.len() && bytes[0..UTF16LE_BOM.len()] == UTF16LE_BOM {
                Ok(String::from_utf16_lossy(unsafe {
                    // SAFETY: We know the pointer and length are valid, since they belong to
                    //         our allocated vector.
